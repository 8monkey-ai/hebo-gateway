...
export const embeddings = (config: GatewayConfig): Endpoint => {
  ...
  const handler = async (ctx: GatewayContext) => {
    ...
    logger.debug({ msg: `[embeddings] model resolved: ${ctx.modelId} -> ${ctx.resolvedModelId}` });
    ...
    logger.debug({
      msg: `[embeddings] provider resolved: ${ctx.resolvedModelId} -> ${embeddingModel.provider}`,
    });
