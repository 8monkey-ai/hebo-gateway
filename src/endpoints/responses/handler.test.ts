...
import { type ResponsesResponse } from "./schema";
...
describe("Responses Handler", () => {
  ...
  test("should generate non-streaming response with string input", async () => {
    ...
    const res = await endpoint.handler(request);
    const data = await parseResponse<ResponsesResponse>(res);
    expect(data).toMatchObject({
    ...
    expect(data!.output.some((item) => item.type === "message")).toBe(true);

    const msgItem = data!.output.find((item) => item.type === "message");
    expect(msgItem).toMatchObject({
      type: "message",
      role: "assistant",
      status: "completed",
    });
  ...
  test("should generate non-streaming response with message items", async () => {
    const request = postJson(baseUrl, {
      model: "openai/gpt-oss-20b",
  ...
  test("should generate response with tool calls", async () => {
    ...
    const fcItem = data!.output.find((item) => item.type === "function_call");
  ...
  test("should accept reasoning parameter", async () => {
    const request = postJson(baseUrl, {
      model: "openai/gpt-oss-20b",
      input: "hi",
      reasoning: {
  ...
  test("should include usage in response", async () => {
    const request = postJson(baseUrl, {
      model: "openai/gpt-oss-20b",
      input: "hi",
    });
