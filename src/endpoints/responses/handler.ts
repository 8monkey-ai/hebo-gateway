...
import { convertToTextCallOptions, toResponse_, toResponsesStream } from "./converters";
...
export const responses = (config: GatewayConfig): Endpoint => {
  ...
  const handler = async (ctx: GatewayContext, cfg: GatewayConfigParsed) => {
    ...
    if (stream) {
      ...
      const result = streamText({
        ...
        onFinish: (res) => {
          ...
          const streamResult = toResponse_(
            res as unknown as GenerateTextResult<ToolSet, Output.Output>,
            ctx.resolvedModelId!,
          );
          logger.trace({ requestId: ctx.requestId, result: streamResult }, "[responses] Response");
      ...
      ctx.result = toResponsesStream(result, ctx.resolvedModelId);
    ...
    ctx.result = toResponse_(result, ctx.resolvedModelId);
    logger.trace({ requestId: ctx.requestId, result: ctx.result }, "[responses] Response");
    addSpanEvent("hebo.result.transformed");
