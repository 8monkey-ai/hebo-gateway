...
// GPT-5.5 onwards are also served by Bedrock, on its `bedrock-mantle` endpoint.
...
export const gpt = {
  ...gptAtomic,
  ...gptGroups,
  // 5.6 replaces the size suffixes with named tiers (Sol/Terra/Luna), so the whole
  // generation lands in `latest`. It ships no Pro variant yet, so 5.5 Pro stays until
  // a 5.6 equivalent exists.
  latest: [...gptAtomic["v5.6"], gpt55Pro],
