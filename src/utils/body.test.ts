...
describe("parseRequestBody", () => {
  ...
  test("allows compressed body within size limit", async () => {
    const payload = { ok: true };
    const compressed = await compress(payload, "gzip");
    const result = await parseRequestBody(gzipRequest(compressed), 10000);
    expect(result).toEqual(payload);
  });

  test("disables decompressed size limit when maxBodySize is 0", async () => {
    const payload = { data: "x".repeat(5000) };
    const compressed = await compress(payload, "gzip");
    const result = await parseRequestBody(gzipRequest(compressed), 0);
    expect(result).toEqual(payload);
  });
